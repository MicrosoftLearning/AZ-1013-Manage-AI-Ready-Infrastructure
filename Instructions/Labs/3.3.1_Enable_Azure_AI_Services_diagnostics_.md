---
lab:
  title: 'AI Ready Manage hands-on exercise: Enable diagnostics for Azure AI Services'
  description: Diagnostic settings in Microsoft Foundry enable administrators to collect
    and route operational data—such as logs and metrics—from Foundry resources to
    destinations like Log Analytics workspaces, Azure Storage accounts, or Event Hubs.
    These settings support centralized monitoring, troubleshooting, and auditing across
    the Foundry environment, allowing consistent data collection and analysis. When
    configuring diagnostic settings, administrators can select specific log and metric
    categories or predefined category groups (such as audit and allLogs) that automatically
    include relevant log types for each resource.
  duration: 25 minutes
  level: 500
  islab: true
  primarytopics:
...
- **Familiarity with Microsoft Foundry resource types**: To learn more, refer to [Choose an Azure resource type for AI foundry](https://learn.microsoft.com/en-us/azure/ai-foundry/concepts/resource-types?view=foundry-classic).

## Estimated duration
25 minutes

### Task 1: Create a Microsoft Foundry hub
...
1. In the Azure portal, use the **Search** text box at the top of the page to search for **Microsoft Foundry** and, in the list of results, select **Microsoft Foundry**.
...
### Task 2: Create a Microsoft Foundry hub-based project
...
### Task 3: Create a Microsoft Foundry project

1. In the web browser displaying the project deployment status, use the **Search** text box at the top of the page to search for **Microsoft Foundry** and, in the list of results, select **Microsoft Foundry**.
...
1. On the **Basics** tab of the **Create storage account** page, specify the following settings (leave others with their default values) and select **Next**:
   ...
   |Region|The name of the Azure region where you created the Microsoft Foundry resources earlier in this exercise|
...
### Task 6: Configure diagnostics for Microsoft Foundry resources

1. In the web browser displaying the deployment status page of the Log Analytics workspace in the Azure portal, use the **Search** text box at the top of the page to search for **Microsoft Foundry** and, in the list of results, select **Microsoft Foundry**.
1. On the **Microsoft Foundry** page, in the vertical menu on the left side, select **Use with Foundry** and then select **AI Hubs**.
1. On the **Microsoft Foundry \| AI Hubs** page, select **diagnostics-hub**.
...
- In the **Diagnostic setting name** text box, enter **Microsoft Foundry hub full diagnostics**
...
1. In the web browser, navigate back to the **Microsoft Foundry** page, in the vertical menu on the left side, select **Use with Foundry** and then select **AI Hubs**.
1. On the **Microsoft Foundry \| AI Hubs** page, select **diagnostics-hub-project**.
...
- In the **Diagnostic setting name** text box, enter **Microsoft Foundry hub project full diagnostics**
...
1. In the web browser, navigate back to the **Microsoft Foundry** page, in the vertical menu on the left side, select **Use with Foundry** and then select **Foundry**.
1. On the **Microsoft Foundry \| Foundry** page, select **diagnostics-project-resource**.
...
- In the **Diagnostic setting name** text box, enter **Microsoft Foundry project full diagnostics**
- In the **Logs** section, select **audit** and **allLogs** category groups. This will automatically select all log categories. 
- In the **Metrics** section, select **AllMetrics**.
- In the **Destination details** section, select the **Send to Log Analytics workspace** checkbox, ensure that the Azure subscription you are using in this exercise appears in the **Subscription** drop-down list and, in the **Log Analytics workspace**, select **ai-foundry-la-workspace**.
- In the **Destination details** section, select the **Archive to a storage account** checkbox, ensure that the Azure subscription you are using in this exercise appears in the **Subscription** drop-down list and, in the **Log Analytics workspace**, select the name of the storage account you created earlier in this exercise.
...
1. In the web browser window displaying the Microsoft Foundry portal, switch back to the tab displaying the Azure portal.
