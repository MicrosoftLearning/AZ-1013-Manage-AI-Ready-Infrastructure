---
lab:
  title: 'AI Ready Manage hands-on exercise: Create a compute instance in a Microsoft
    AI Foundry hub'
  description: 'Compute resources in Microsoft AI Foundry provide the processing power
    required to execute workloads such as model training, data preparation, and experimentation.
    Within this environment, compute instances serve as dedicated, user-assigned virtual
    machines designed for interactive development and job execution. Each compute
    instance is assigned to a single user and cannot be shared. It can be reused across
    multiple workflows within that user''s workspace. Compute instances are currently
    supported only in hub-based projects—standalone Microsoft AI Foundry projects
...
# AI Ready Manage hands-on exercise: Create a compute instance in a Microsoft AI Foundry hub

## Background information
Compute resources in Microsoft AI Foundry provide the processing power required to execute workloads such as model training, data preparation, and experimentation. Within this environment, compute instances serve as dedicated, user-assigned virtual machines designed for interactive development and job execution. Each compute instance is assigned to a single user and cannot be shared. It can be reused across multiple workflows within that user's workspace. Compute instances are currently supported only in hub-based projects—standalone Microsoft AI Foundry projects do not include this feature. You need a compute instance to:
...
### Task 2: Create a compute instance in a Microsoft Foundry hub

1. In the web browser displaying the hub's **Overview** page in the Azure portal, select **Launch Microsoft Foundry**. 

   **Note**: This should automatically open another tab in the web browser window displaying the **Overview** page of the hub in the Microsoft Foundry portal.

1. In the web browser displaying the Microsoft Foundry portal, on the **Overview** page of **Compute hub**, in the vertical menu on the left side, select **Compute**. 
